...
import { useGetAudit } from "@/app/hooks/useGetAudit";
import { useAuditVoting } from "@/app/hooks/useAuditVoting";
...
export default function Bounty() {
  const { id } = useParams();
  const { dataAudit, isLoading, isError, address } = useGetAudit(id.toString());
  const { handleUpvote } = useAuditVoting();

  if (!address) {
    return <h3>Wallet not connected</h3>;
  }

  const onUpvote = async (bountyId: string) => {
    const res = await handleUpvote(bountyId, id.toString());
    console.log("upvoteAudit: " + res);
      ...
      <div className="m-5 space-y-3">
        ...
        <div className="relative z-0 w-full mb-6 group">
          <input
            type="text"
            className="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer"
            value={`${dataAudit?.bounty_id}`}
            disabled
          />
          <label className="peer-focus:font-medium absolute text-sm text-gray-500 dark:text-gray-400 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:start-0 rtl:peer-focus:translate-x-1/4 rtl:peer-focus:left-auto peer-focus:text-blue-600 peer-focus:dark:text-blue-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">{`Bounty ID:`}</label>
        </div>
        <div className="grid md:grid-cols-2 md:gap-6">
          <div className="relative z-0 w-full mb-6 group">
            <input
              ...
              className="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer"
              value={`${new Date(
                (dataAudit?.time_issued as number) / 1,
              ).toLocaleString()}`}
              disabled
            />
        ...
        <div className="grid md:grid-cols-3 md:gap-6">
          <div className="relative z-0 w-full mb-6 group">
            <input
              type="text"
              className="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer"
              value={`${dataAudit?.severity}`}
              disabled
            />
            <label className="peer-focus:font-medium absolute text-sm text-gray-500 dark:text-gray-400 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:start-0 rtl:peer-focus:translate-x-1/4 rtl:peer-focus:left-auto peer-focus:text-blue-600 peer-focus:dark:text-blue-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">{`Severity:`}</label>
          </div>
        </div>
        ...
        <div className="flex flex-row space-x-6">
          <Button
            color="green"
            onClick={() => onUpvote(`${dataAudit?.bounty_id}`)}
          >
            Up Vote
